...
<div class="container-fluid">
    <div class="row">
        ...
        <nav class="col-md-2 d-none d-md-block sidebar">
            ...
            <a href="pengguna.php" class="active"><i class="fa fa-users me-2"></i> Pengguna</a>
        ...
        <main class="col-md-10 ms-sm-auto content">
            <div class="table-container mb-4">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalTambah"><i class="fa fa-plus"></i> Tambah Pengguna</button>
                </div>
                ...
                <?php endif; ?>
                <table id="tabelPengguna" class="table table-striped">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Nama</th>
                    ...
                    <tbody>
                        <?php
                        $no = 1;
                        $query = mysqli_query($koneksi, "SELECT * FROM user ORDER BY user_id DESC");
                        while($data = mysqli_fetch_assoc($query)):
                        ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td><?= htmlspecialchars($data['user_nama']) ?></td>
                            <td><?= htmlspecialchars($data['user_username']) ?></td>
                            <td><span class="badge bg-<?= $data['user_level']=='administrator'?'primary':'secondary' ?>"><?= ucfirst($data['user_level']) ?></span></td>
                            <td>
                                <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#modalEdit<?= $data['user_id'] ?>" title="Edit"><i class="fa fa-edit"></i></button>
                                <a href="pengguna_aksi.php?hapus=<?= $data['user_id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus data ini?')" title="Hapus"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                        <!-- Modal Edit -->
                        <div class="modal fade" id="modalEdit<?= $data['user_id'] ?>" tabindex="-1">
                                ...
                                <div class="modal-content">
                                    ...
                                    <form action="pengguna_aksi.php" method="POST">
                                        <div class="modal-body">
                                            <input type="hidden" name="pengguna_id" value="<?= $data['user_id'] ?>">
                                            <div class="mb-3">
                                                <label class="form-label">Nama</label>
                                                <input type="text" class="form-control" name="nama" value="<?= htmlspecialchars($data['user_nama']) ?>" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">Username</label>
                                                <input type="text" class="form-control" name="username" value="<?= htmlspecialchars($data['user_username']) ?>" required>
                                            </div>
                                            <div class="mb-3">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="level" class="form-control" required>
                                                    <option value="administrator" <?= $data['user_level']=='administrator'?'selected':'' ?>>Administrator</option>
                                                    <option value="manajemen" <?= $data['user_level']=='manajemen'?'selected':'' ?>>Manajemen</option>
                                                </select>
                                            </div>
                                        </div>
